...
export function ExamBuilder({ mode, classes, questions, exam }: Props) {
  ...
  // Derived
  const pool = questions.filter((q) => q.class_id === classId && !pickedIds.includes(q.id));
  const filteredPool = pool.filter(
    (q) => poolSearch === "" || q.text.toLowerCase().includes(poolSearch.toLowerCase()),
  );
  ...
  async function handleSave(action: "draft" | "publish") {
    ...
    if (action === "publish") {
      ...
      const windowMs =
        new Date(myLocalInputToISO(endTime)).getTime() -
        new Date(myLocalInputToISO(startTime)).getTime();
    ...
    try {
      if (mode === "new") {
        ...
        toast.success(action === "publish" ? "Exam published!" : "Draft saved!");
      } else if (exam) {
        await updateExam({
          data: {
            id: exam.id,
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border-2 border-ink bg-card p-6 shadow-brut">
            ...
            {/* Camera toggle */}
            <div
              className={`flex items-center justify-between px-4 py-3 rounded-2xl border-2 transition-colors ${requireCamera ? "border-violet bg-violet/10" : "border-ink/20 bg-background"}`}
            >
              <div>
            ...
            {/* Shuffle toggle */}
            <div
              className={`flex items-center justify-between px-4 py-3 rounded-2xl border-2 transition-colors ${shuffle ? "border-violet bg-violet/10" : "border-ink/20 bg-background"}`}
            >
              <div>
                <p className="text-xs font-mono uppercase tracking-widest text-ink/60">
                  Shuffle questions
                ...
                <p className="text-sm font-semibold mt-0.5">
                  {shuffle ? "Per-student random order + options" : "Same order for everyone"}
                </p>
              </div>
              <button
                type="button"
              ...
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white border border-ink/30 transition-all ${shuffle ? "left-6" : "left-0.5"}`}
                />
              </button>
            </div>
          </div>
          ...
          <div
            className={`rounded-3xl border-2 border-ink bg-card p-6 shadow-brut ${isPublished ? "opacity-60 pointer-events-none select-none" : ""}`}
          >
            ...
            {pickedQs.length === 0 ? (
              ...
              <ul className="space-y-2">
                {pickedQs.map((q, idx) => (
                  ...
                  >
                    <span className="text-xs font-mono text-ink/40 w-5">{idx + 1}</span>
                    <span
                    ...
                    <span className="text-xs font-mono text-ink/60">{q.points}pt</span>
                    <button
                      type="button"
                ...
                {filteredPool.map((q) => (
                  <li key={q.id}>
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs font-mono text-ink/60">{q.points}pt</span>
                    </button>
                  </li>
                ))}
              </ul>
