...
function LastSeen({ iso, active }: { iso: string | null; active: boolean }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 text-xs font-mono ${stale ? "text-pink font-bold" : "text-green-700"}`}
    >
      {stale ? (
    ...
    <div className="mt-2 space-y-1">
      {hard.map((f, i) => {
        ...
        return (
          <div
            key={i}
            className="flex items-center gap-2 text-xs font-mono px-2.5 py-1.5 rounded-lg bg-pink/15 text-pink border border-pink/20"
          >
    ...
    <>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-4">
        {[
          ...
          <div
            key={label}
            className={`rounded-2xl border-2 border-ink p-3 text-center shadow-brut-sm ${color}`}
          >
        ...
        <div
          className={`rounded-2xl border-2 border-ink p-3 text-center shadow-brut-sm ${examEnded ? "bg-secondary" : "bg-sky/20"}`}
        >
      ...
      <div className="mb-6 rounded-2xl border-2 border-ink bg-card px-5 py-3 shadow-brut-sm">
        <div className="flex justify-between items-center mb-2 text-xs font-mono">
          ...
          <span className="font-bold">
            {completedCount} / {totalEnrolled} &nbsp;({completionPct}%)
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mt-2 text-[10px] font-mono text-muted-foreground">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-secondary border border-ink/30 inline-block" />
            ...
            return (
              <Card key={s.id} className={isFlagged ? "border-pink" : ""}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 rounded-full border-2 border-ink flex items-center justify-center font-bold text-sm shrink-0 ${isFlagged ? "bg-pink text-white" : isSubmitted ? "bg-lime" : "bg-sky"}`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      ...
                      <span
                        className={`text-xs font-mono font-bold uppercase ${isFlagged ? "text-pink" : isSubmitted ? "text-green-700" : "text-amber-700"}`}
                      >
                ...
                {/* Status panel */}
                <div
                  className={`rounded-2xl border-2 border-ink p-3 flex flex-col gap-2.5 ${isFlagged ? "bg-pink/10" : isSubmitted ? "bg-lime/10" : "bg-secondary"}`}
                >
                  {/* Flags bar */}
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-xs font-mono font-bold ${flags >= INTEGRITY.FLAG_THRESHOLD ? "text-pink" : flags > 0 ? "text-amber-700" : "text-muted-foreground"}`}
                      >
        ...
        <div className="rounded-3xl border-2 border-ink bg-card p-6 shadow-brut">
          ...
          <div className="flex flex-wrap gap-2">
            {notStarted.map((name: string, i: number) => (
              <span
                key={i}
                className="px-3 py-1.5 rounded-full border-2 border-ink/20 bg-secondary text-sm font-mono"
              >
                {name}
              </span>
            ))}
      ...
      {submissions.length === 0 && (notStarted?.length ?? 0) === 0 && (
        <Card className="text-center py-12 text-ink/50">No students enrolled in this class.</Card>
      )}
    </>
  );
}
