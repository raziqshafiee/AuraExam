        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 bg-card border-2 border-ink rounded-xl shadow-brut-sm overflow-hidden min-w-[140px]">
            <button
              ...
              className="block w-full px-4 py-2.5 text-sm text-left hover:bg-secondary font-mono"
            >
              ↓ CSV
            </button>
            <button
              ...
              className="block w-full px-4 py-2.5 text-sm text-left hover:bg-secondary font-mono border-t border-ink/10"
            >
              ⎙ Print / PDF
            </button>
          </div>
...
function Results() {
  ...
  useEffect(() => {
    setSubs(initial.submissions as Submission[]);
  }, [initial]);
  ...
  const submitted = useMemo(
    () => subs.filter((s) => s.status !== "in-progress" && s.status !== "not-answered"),
    [subs],
  );
  ...
  const essayPending = useMemo(
    () => subs.filter((s) => s.essayAnswers.some((ea) => ea.score === null)),
    [subs],
  );

  const scoredSubs = useMemo(
    () => submitted.filter((s) => s.score !== null && s.total),
    [submitted],
  );

  const highScore = useMemo(() => {
    if (!scoredSubs.length) return null;
    return scoredSubs.reduce((best, s) =>
  ...
  const lowScore = useMemo(() => {
    if (!scoredSubs.length) return null;
    return scoredSubs.reduce((worst, s) =>
      s.score! / s.total! < worst.score! / worst.total! ? s : worst,
    );
  }, [scoredSubs]);
  ...
  function exportExamCSV() {
    ...
    const rows = subs.map((s) => [
      ...
      s.score ?? 0,
      s.total ?? 0,
      pct(s.score, s.total ?? 0),
      s.status,
      s.auto_score,
      Math.max(0, (s.score ?? 0) - s.auto_score),
      s.flags,
      fmtExport(s.submittedAt),
    ]);
  ...
  function exportExamPDF() {
    ...
    printTable(
      `${exam.title} — Results`,
      `${exam.classCode} · ${submitted.length} submissions`,
      headers,
      rows,
    );
  }
  ...
  function exportSlipPDF(s: Submission) {
    ...
    printTable(
      `Result Slip — ${s.studentName}`,
      `${exam.title} · ${exam.classCode}`,
      headers,
      rows,
    );
  }
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={`w-9 h-9 rounded-full border-2 border-ink flex items-center justify-center font-bold text-sm shrink-0 ${
                            s.status === "flagged"
                              ? "bg-pink text-white"
                              : s.status === "graded"
                                ? "bg-lime"
                      ...
                      <span
                        className={`shrink-0 px-2 py-0.5 rounded-full border border-ink/30 text-[10px] font-mono uppercase tracking-wide ${STATUS_COLORS[s.status] ?? "bg-card"}`}
                      >
                        {s.status.replace("-", " ")}
                      </span>
                    </div>
                    ...
                    {!isNA && (
                      <div>
                        <div className="flex justify-between items-baseline mb-1.5 text-sm font-mono">
                          ...
                          {scorePct !== null && (
                            <span
                              className={`font-bold ${scorePct >= 50 ? "text-green-700" : "text-pink"}`}
                            >
                              {scorePct}%
                            </span>
                          )}
                      ...
                      <div className="flex items-center gap-2 text-xs font-mono">
                        {s.flags > 0 ? (
                          <span className="flex items-center gap-1 text-pink font-bold">
                            <AlertTriangle className="w-3.5 h-3.5" /> {s.flags} flag
                            {s.flags !== 1 ? "s" : ""}
                          </span>
                        ) : (
                      ...
                      >
                        <div className="text-left">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {allGraded ? (
                              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-lime border border-ink/20 uppercase">
                                Done
                              </span>
                            ) : (
                              <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-amber border border-ink/20 uppercase">
                                {s.essayAnswers.length - gradedCount} pending
                              </span>
                            )}
                          </div>
                          <div className="text-xs font-mono text-muted-foreground mt-0.5">
                            {s.essayAnswers.length} essay question
                            {s.essayAnswers.length !== 1 ? "s" : ""} · {gradedCount}/
                            {s.essayAnswers.length} graded
                          </div>
                        </div>
                        {isOpen ? (
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {ea.score !== null && (
                                    <span className="px-2 py-0.5 rounded-full bg-lime border-2 border-ink text-xs font-mono font-bold">
                                      {ea.score} / {ea.maxPoints} pts ·{" "}
                                      {Math.round((ea.score / ea.maxPoints) * 100)}%
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="space-y-1.5">
                                  <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    <input
                                      ...
                                      max={ea.maxPoints}
                                      value={
                                        scores[key] ?? (ea.score !== null ? String(ea.score) : "")
                                      }
                                      onChange={(ev) => {
                                  ...
                                  {scoreErrors[key] && (
                                    <p className="text-xs font-mono text-pink font-semibold pl-[72px]">
                                      {scoreErrors[key]}
                                    </p>
                                  )}
                                </div>
